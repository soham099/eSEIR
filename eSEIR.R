...
tvt.eSEIR <- function (Y,R, pi0=NULL,change_time=NULL,exponential=FALSE,lambda0=NULL,begin_str="01/13/2020",T_fin=200,nchain=4,nadapt=1e4,M=5e2,thn=10,nburnin=2e2,dic=FALSE,death_in_R=0.02,beta0=0.2586,gamma0=0.0821,sigma0=1/5.2, R0=beta0/gamma0,gamma0_sd=0.1, R0_sd=1,  sigma0_sd=0.3, casename="tvt.eSIR",file_add=character(0),add_death=FALSE,save_files=FALSE,save_mcmc=FALSE,save_plot_data=FALSE,eps=1e-10)
  ...
  plot1 <- ggplot(data = data_poly, aes(x = x, y = y)) +
    ...
    labs(title=substitute(paste(casename,": infection forecast with prior ",beta[0],"=",v1,",",sigma0, "=", v4,",",gamma[0], "=",v2," and ", R[0],"=",v3), list(casename=casename,v1=format(beta0,digits=3),v2=format(gamma0,digits=3),v4=format(sigma0,digits=3),v3=format(R0,digits=3))),subtitle = substitute(paste("Posterior ", beta[p],"=",v1,",",sigma[p], "=", v4,",",gamma[p], "=",v2," and ", R[0],"=",v3), list(v1=format(beta_p_mean,digits=3),v2=format(gamma_p_mean,digits=3),v4=format(sigma_p_mean,digits=3),v3=format(R0_p_mean,digits=3))),x = "time", y = "P(Infected)")+
...
res <-tvt.eSEIR(Y,R,begin_str="01/13/2020",death_in_R = 0.4,T_fin=200,
